import styles from './WizardSteps.module.css';
import type { SimulatorInputs } from '../../types';
import { formatKRWShort, formatPercent } from '../../lib/format';
import { resolveBusinessParams } from '../../lib/calculator';

interface Props {
  inputs: SimulatorInputs;
  onCalculate: () => void;
}

export function ConfirmStep({ inputs, onCalculate }: Props) {
  const { business_type: bt, scale, capital } = inputs;
  const params = resolveBusinessParams(inputs);
  const debt = Math.max(0, capital.initial_investment - capital.equity);

  const rows: { label: string; value: string }[] = [
    { label: '업종', value: bt.name },
    { label: '규모', value: scale === 'small' ? '소형' : scale === 'large' ? '대형' : '중형' },
    { label: '초기투자금', value: formatKRWShort(capital.initial_investment) },
    { label: '자기자본', value: formatKRWShort(capital.equity) },
    { label: '대출', value: formatKRWShort(debt) },
    { label: '금리', value: formatPercent(capital.interest_rate) },
    { label: '대출기간', value: `${capital.loan_term_years}년` },
    {
      label: '일 방문객',
      value: `${inputs.daily_customers_override ?? (scale === 'small' ? bt.avg_daily_customers_small : scale === 'large' ? bt.avg_daily_customers_large : bt.avg_daily_customers_medium)}명`,
    },
    { label: '객단가', value: formatKRWShort(params.avg_ticket_price) },
    { label: '직원 수', value: `${inputs.labor_headcount ?? 1}명` },
    { label: '월 임대료', value: formatKRWShort(inputs.rent_monthly ?? 0) },
  ];

  return (
    <div className={styles.step}>
      <h2 className={styles.stepTitle}>입력 내용 확인</h2>
      <p className={styles.stepDesc}>내용을 확인하고 계산하기를 눌러주세요</p>
      <div className={styles.summarySection}>
        {rows.map((row, i) => (
          <div key={i} className={styles.summaryRow}>
            <span className={styles.summaryLabel}>{row.label}</span>
            <span className={styles.summaryValue}>{row.value}</span>
          </div>
        ))}
      </div>
      <div className={styles.disclaimer}>
        본 시뮬레이션은 참고용이며, 부가가치세(VAT)는 반영되지 않았습니다.
        실제 수익은 입지, 경쟁, 운영 역량 등에 따라 크게 달라질 수 있습니다.
      </div>
      <button className={styles.nextBtn} onClick={onCalculate}>계산하기</button>
    </div>
  );
}
